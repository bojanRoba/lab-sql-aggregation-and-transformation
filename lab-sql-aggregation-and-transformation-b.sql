...
FROM sakila.film ORDER BY title ASC;
...
SELECT rating, COUNT(rating) as rating_num
...
SELECT rating, COUNT(rating) as rating_num
FROM sakila.film GROUP BY rating ORDER BY rating_num DESC;
...
FROM sakila.film GROUP BY rating ORDER BY avg_length DESC;

-- 2.2 Identify which ratings have a mean duration of over two hours in order to help select films for customers who prefer longer movies.

SELECT rating, ROUND(AVG(length),2) as avg_length
FROM sakila.film GROUP BY rating HAVING avg_length > 120;
